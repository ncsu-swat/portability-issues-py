...
Our fix creates a cross-platform \CodeIn{is\_admin()} function that uses Windows API for administrator checks on Windows and \CodeIn{os.geteuid()} on Unix-like systems. This exemplifies three fix strategies: \textbf{Environment handling} (platform-specific conditional logic), \textbf{Portable APIs} (OS detection), and \textbf{Defensive checks} (fallback handling).
