...
In the second configuration (\textit{pattern-guided prompt}), each non-portable snippet was accompanied by a description of the specific portability issue and the corresponding \textit{General Fix Pattern} from Table~\ref{tab:symptoms}.  
...
\begin{tcolorbox}[boxrule=0pt,sharp corners,boxsep=2pt,left=2pt,right=2pt,top=2.5pt,bottom=2pt]
\textbf{RQ\textsubscript{3} (detection and repair)}: Traditional static analysis tools do not offer support for portability detection—only Ruff provides a specific rule addressing encoding issue. LLMs show stronger potential: in the detection task, they achieved solid recall and precision, with the best model reaching around 70\%. However, their ability to fix portability issues remains limited. When asked to repair 30 faulty snippets, success rates ranged from 33-43\% with generic prompts, and improved to 50-77\% only when guided by explicit issue descriptions and fix patterns. Overall, LLMs can recognize and sometimes repair portability problems, but reliable automated correction still demands structured guidance or integration with static verification tools.
\end{tcolorbox}
...
\textbf{Quick Answer.} \Fix{add in a box} Fixes often involve replacing OS-specific APIs, adding conditional checks, or adopting cross-platform libraries.  
