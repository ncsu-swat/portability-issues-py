...
\Den{cross-OS} portability issues in Python, analyzing \NumProjectsTotal{}
...
\Den{cross-OS} portability issues in Python, providing actionable insights
...
\sloppy
This paper presents a study of \Den{cross-OS} portability issues in Python
...
by two co-authors, with regular discussions to resolve 
...
had reached theoretical saturation. \Den{(need some cite?)}
...
90 repair attempts per configuration. \Den{New:} We also examine repair performance 
...
\begin{table}[t]
\centering
\caption{\Den{New table:}Pattern-guided repair performance breakdown: cases with symptom descriptions vs.\ empty symptom fields.}
...
\Den{New:}A closer examination of the pattern-guided results (Table~\ref{tab:llm_fix_results_symptom_breakdown}) reveals that the availability of symptom descriptions significantly influences LLM performance.  
...
\Den{New: updated as RBQ2 requested}
...
Al Awar \etal{} present PyKokkos~\cite{al2021performance}, a Python framework that attains performance portability by translating Python kernels into optimized C++ Kokkos code~\cite{kokkos}. Their approach enables near-native execution while keeping development in Python. Like other performance-portable systems, PyKokkos addresses architectural efficiency, whereas our work focuses on OS-level inconsistencies across operating systems.
...
\Den{cross-OS} portability issues in Python code. Across 900 projects, we found that
...
\Den{Updated:}
The artifacts --including datasets, submitted PRs and scripts-- are publicly available: \ourrepo
