...
90 repair attempts per configuration.
...
issue, we apply one of the four fix patterns from our
taxonomy~(Table~\ref{tab:symptoms}), tested the fix locally on the
affected platforms, and submitted a pull request with a detailed
explanation of the portability problem and the proposed solution.
...
itself to ensure consistent behavior across platforms.

\Mar{Is there a reason for the two discussed PR to be of the same kind? If you think there is are good representative, justify it.}
\Den{I think it's fine to have both examples from the same category, as long as they illustrate different sub-categories and fix patterns. The FILE category is the most prevalent, so it makes sense to highlight examples from there. Please review the next paragraph.}
...
patterns address them. Both examples are from the File and Directories (FILE)
category, which represents the most prevalent portability issue (62 projects, 41\% of all issues). 
These cases demonstrate different 
sub-categories (file locking and path separators) and showcase how the same 
fix pattern (Environment handling) addresses distinct underlying problems.
...
exemplifies the \textbf{Environment handling} fix strategy from our
taxonomy, where platform-specific behavior is accommodated through
...
\begin{lstlisting}[caption={Patch for cross-drive relative path issue.}, label={lst:pydantic_fix}]
 @pytest.fixture
...
We demonstrate another portability fix through the project~\cite{hosteuropeletsencrypt}, which has 65 stars and involves system administration functionality. 

The problem we encountered falls into the \textbf{API Availability (API)} category, specifically the ``Methods: os.uname(), os.geteuid(), os.getuid(), os.getpgid()'' sub-category. The issue occurred where \CodeIn{os.geteuid()} was used to determine if the script was running with root privileges. This function is unavailable on Windows distributions of Python, causing \texttt{AttributeError: module `os' has no attribute `geteuid'} when the code is executed on Windows systems.

Listing~\ref{lst:shared_fix} shows the diff of our fix applied to the shared module. The original code directly used the Unix-specific \CodeIn{os.geteuid()} function to check for root privileges. However, this approach fails on Windows, where the concept of ``root'' doesn't exist and the function is not available in the \texttt{os} module.
...
Our fix addresses this issue by creating a cross-platform \CodeIn{is_admin()} function that uses platform detection to choose the appropriate method for checking administrative privileges. On Windows systems, it uses the Windows API through \CodeIn{ctypes.windll.shell32.IsUserAnAdmin()} to determine if the current user has administrator privileges. On Unix-like systems (Linux/macOS), it maintains the original \CodeIn{os.geteuid() == 0} approach. The function includes error handling to gracefully fall back to \CodeIn{False} if either method fails. This pattern exemplifies the \textbf{Portable APIs + Defensive checks} fix strategy from our taxonomy, where platform-specific APIs are wrapped with conditional logic and appropriate fallbacks.

This case demonstrates how administrative privilege detection requires fundamentally different approaches across operating systems, with Windows using user access tokens and Unix systems using effective user IDs. The fix preserves the original functionality while extending compatibility to all major platforms.
