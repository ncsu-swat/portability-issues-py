...
accepted at the time of writing. The overall acceptance rate of \TotalPRAprovedPorcent{},
...
\noindent\textbf{Internal validity:} Our cross-OS test re-execution relies on GitHub Actions runners, which may differ from physical machines. Manual GitHub issue analysis involved subjective interpretation. 
\emph{Mitigation:} We used standardized GitHub runners across three OS configurations and conducted manual analysis independently by multiple researchers with consensus validation.
...
\emph{Mitigation:} We sampled projects across diverse domains and sizes, our taxonomy emerges from fundamental OS differences that transcend specific domains, and our high pull request acceptance rate validates findings across multiple communities.
...
\emph{Mitigation:} We validated our taxonomy through systematic comparison across two independent data sources and community validation via pull requests.
...
\emph{Mitigation:} We triangulated findings across multiple independent methods, validated taxonomy through both detection approaches, and achieved \TotalPRAprovedPorcent{} pull request acceptance with zero rejections.
...
Our taxonomy identifies 7 root-cause categories with 24 sub-categories and 4 general fix patterns. LLMs achieve 40-79\% detection accuracy and 50-77\% repair success when guided by structured patterns. Our findings are validated through 33 pull requests with \TotalPRAprovedPorcent{} acceptance rate.
