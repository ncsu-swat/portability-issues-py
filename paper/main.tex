...
portability issues in Python, analyzing \NumProjectsTotal{}
...
OS-dependent test failures, with \NumTestRunsWithOSDiff{}
...
portability detection, while large language models achieve 65--79\% 
accuracy in identifying issues and 40--77\% success in generating fixes
...
Python portability issues, providing actionable insights
...
containerized. In our analysis of \NumProjectsTotal{} \github\ repositories,
...
\begin{packed_itemize}
  ...
  \item[\Contrib] A dataset of \Fix{xxx} portability issues across \Fix{yyy}
    Python projects.
  \item \Fix{Add our artifact link here? \ourrepo{}}
\end{packed_itemize}
...
with similar underlying causes. This process was conducted by the
authors, with regular discussions to resolve ambiguities and refine
...
\Ali{Updated with new content, please review this section.}
\noindent Understanding how portability issues manifest during execution 
is essential for rapid diagnosis and triaging. In this section, we examine 
the observable symptoms of portability problems—the concrete error messages, 
exceptions, and behavioral differences that appear in test logs, CI output, 
and runtime failures. Table~\ref{tab:symptoms} maps each portability 
sub-category to its characteristic symptom signature --when uniquely identifiable--
and corresponding fix pattern.
...
\noindent\begin{table*}[t]
    ...
    \midrule
    \textbf{LIB} &
    Platform-specific libraries (e.g. \CodeIn{fcntl}) & \UniqueSymptom{\texttt{ModuleNotFoundError: No module named `fcntl'} (Windows)} & \DefensivePattern{} — conditional import provide fallback \\
    ...
    \UniqueSymptom{\texttt{AttributeError: module `os' has no attribute `'\{method\}'} (Windows)} & \PortablePattern{} + \DefensivePattern{} — use platform.uname, hasattr guards \\
    & Modules: readline, resource & \UniqueSymptom{\texttt{ModuleNotFoundError: No module named `\{module\}'} (Windows)} & \DefensivePattern{} — guard import use alternatives \\
    \midrule
    \textbf{ENV} &
    No display in github CI (linux) & \UniqueSymptom{\texttt{TclError: no display name and no \$DISPLAY environment variable}} & \EnvPattern{} — skip GUI tests in CI \\
    ...
    \midrule
    \textbf{SYS} &
    /proc filesystem & \ContextSymptom{Context-dependent: /proc paths fail on macOS/Windows with generic FileNotFoundError} & \PortablePattern{} — use psutil or platform APIs \\
...
\MyPara{Implications for diagnosis}~Our findings reveal that while unique signatures 
enable rapid automated triage for 62.5\% of portability issues, the remaining 37.5\% require 
more sophisticated analysis. This suggests that fully automated portability 
detection tools must go beyond simple log parsing and incorporate contextual analysis, 
such as static code analysis to detect potential portability problems (\eg{}, hardcoded paths, shell command patterns, or 
platform-specific API usage). The presence of context-dependent symptoms also highlights 
the importance of comprehensive cross-platform testing, as these issues cannot be 
...
without maintaining separate test files for each operating system.
...
ensuring the code works correctly on both Unix-like systems and Windows.
...
these patterns distribute across the 24 portability
sub-categories. Defensive checks and Environment handling are the most
prevalent strategies, each addressing 9 sub-categories (33.3\%),
together accounting for two-thirds of all portability fixes.
Normalization applies to 6 sub-categories (22.2\%), primarily
addressing file and path-related issues. Portable APIs, while used
least frequently (3 sub-categories, 11.1\%), targets specific,
...
sub-categories: Defensive checks and Environment handling are most prevalent (each 
33.3\%), followed by Normalization (22.2\%) and Portable APIs (11.1\%). These patterns 
represent recurring, localized code modifications that provide systematic guidance 
for resolving portability issues across diverse contexts.
\end{tcolorbox}
...
% \\
This code has \texttt{<symptom-description>}.  Consider using one of the 
following fixes \textit{<list-of-possible-fixes-for-the-symptom>}. \\
% \\
...
and Mac (see Table~\ref{tab:llm_prompts} for the exact prompts used). The models' responses were parsed to extract their
...
\noindent\textbf{Metrics.}~We used standard metrics to measure
performance of binary classifiers (e.g., precision, recall, accuracy,
etc.). Table~\ref{tab:llm_detection_results} shows
...
In the first configuration (\textit{generic prompt}), each model received a general instruction stating that the code exhibited portability issues and was asked to produce a corrected version (see Table~\ref{tab:llm_prompts}).  
...
The rationale for this approach is: (1) observe symptoms in the code, (2)
 look up the mapping in Table~\ref{tab:symptoms} (Symptom $\rightarrow$ 
 General Fix Pattern), and (3) prompt the LLM with specific fix pattern 
 guidance. For example, when encountering code that produces 
 \texttt{AttributeError: module `signal' has no attribute `SIGHUP'}, 
 we would prompt: ``This code has \texttt{AttributeError: module `signal' 
 has no attribute `SIGHUP'}. Consider using the \texttt{Defensive checks} 
 pattern: \texttt{guard signals per platform}.'' Table~\ref{tab:llm_prompts} 
 shows the template structure used for these pattern-guided prompts.   
...
\begin{tcolorbox}[boxrule=0pt,sharp corners,boxsep=2pt,left=2pt,right=2pt,top=2.5pt,bottom=2pt]
  \textbf{RQ\textsubscript{3}:}~ The results show that the
  medium-sized LLMs analyzed perform poorly at detecting portability
  issues. Their performance improves in repair tasks when distinct
  symptoms can be clearly associated with specific fixes
...
We applied the cross-OS test re-execution method
(\textsection~\ref{sec:methods}) to detect portability problems in our
application set (\textsection~\ref{sec:projects}). For each identified
issue, we applied one of the four fix patterns from our taxonomy
(Table~\ref{tab:symptoms}), tested the fix locally on the affected
platforms, and submitted a pull request with a detailed explanation of the
portability problem and the proposed solution.\Ali{Minor updates, please review}
...
accepted at the time of writing. The overall acceptance rate of 48\%,
...
\TotalPRInTestsCode{} only the test code changes, in
\TotalPRInProgramCode{} cases only the program code changes, and in
\TotalPRInBothCode{} cases both the test and program code
changes. This distribution suggests that while some portability issues
can be resolved by adjusting tests to be more environment-aware, many
require changes to the application logic itself to ensure consistent
behavior across platforms.

Next, we highlight two representative pull requests that illustrate
how portability issues manifest in practice and how our identified fix 
patterns address them.
...
We illustrate how portability issues manifest and are fixed through the webassets project~\cite{webassets}. Webassets is an asset management library for Python web development with 932 stars, used for bundling and processing web assets like CSS and JavaScript files.
...
Listing~\ref{lst:webassets_fix} shows the diff of our fix applied to the test file. The original code created a temporary file within a context manager that automatically deleted the file upon exiting the \texttt{with} block. However, on Windows, attempting to reopen this file (which the test needed to do because it's a fixture) failed because the file was still locked by the previous file handle.
 %, language=diff

\begin{lstlisting}[caption={Diff showing our approved PR for temporary file handling issue.}, label={lst:webassets_fix}]
 @pytest.fixture
 def tmp_file():
...
Our fix addresses this issue by modifying line 4 to disable automatic deletion using \CodeIn{delete=False}, storing the file path on line 8 before the file handle is closed, and manually cleaning up the temporary file on line 10 after the test completes. This pattern exemplifies the \textbf{Environment handling} fix strategy from our taxonomy, where platform-specific behavior is accommodated through careful resource management.
...
We present another case from the pydantic-extra-types project~\cite{pydantic_extra_types}. Pydantic-extra-types is a library that provides additional type validators for Pydantic with 290 stars, used for extending Pydantic's~\cite{pydantic_validation} validation capabilities with specialized data types.

The problem we encountered also falls into the \textbf{File and Directories (FILE)} category, specifically the ``Path separators'' sub-category. The issue occurred in test fixtures where \CodeIn{os.path.relpath} was used to create relative paths from absolute paths. On Windows, this function fails when the source and target paths are on different drives (e.g., \texttt{C:\textbackslash} vs \texttt{D:\textbackslash}), as Windows cannot create relative paths between different drive letters~\cite{python-relpath}.

Listing~\ref{lst:pydantic_fix} shows our fix applied to test fixtures. The original code assumed that \CodeIn{os.path.relpath} would always succeed, but on Windows this assumption may not hold. For example, when tests are executed from a project directory located on drive \texttt{D:} while the system temporary directory (used to create fixture files) resides on drive \texttt{C:}, the function call fails because it attempts to compute a relative path across drives. Since the location of temporary directories on Windows is determined by environment variables and may differ from the working drive~\cite{python-tempfile}, this behavior can lead to path resolution errors and subsequent test failures.


\begin{lstlisting}[caption={Diff showing our fix for cross-drive relative path issue.}, label={lst:pydantic_fix}]
 @pytest.fixture
...
\Den{new box, please verify:}


\begin{tcolorbox}[boxrule=0pt,sharp corners,boxsep=2pt,left=2pt,right=2pt,top=2.5pt,bottom=2pt]
  \textbf{RQ\textsubscript{4} (community validation):}~ We submitted \TotalPROpened{} pull requests to open-source projects, achieving a 48\% acceptance rate with zero rejections. File and Directories issues were most frequent (15 PRs), while Process and Signals issues had the highest acceptance rate (100\%). The results validate that our identified issues represent genuine portability problems recognized by maintainers, with fixes typically requiring localized changes to either test code (51.5\%) or application logic (30.3\%).
\end{tcolorbox}
...
\Ali{Updated, please review.}

\subsection{Guided LLM Repair Using Error Signatures and Fix Patterns}

\noindent\textbf{Observation.}~LLMs underperform with generic ``fix this code'' prompts
(27--43\% success) but become effective when guided by structure: including the
portability category and a mapped fix pattern increases repair rates to
50--77\% (best: 77\% with Grok-4-Fast). In practice, this guidance is often
available because many portability sub-categories produce distinctive,
actionable error messages that allow immediate classification from logs
(Table~\ref{tab:symptoms}).

\noindent\textbf{Recommendation.}~Use a symptom+pattern workflow when---and only when---a
specific portability symptom from Table~\ref{tab:symptoms} is present. Capture
the OS, the exact error text, and a minimal snippet; then prompt the LLM with
the code plus the symptom and the mapped fix pattern from our taxonomy (\eg{},
Environment handling, Normalization, Portable APIs). Following this approach
when a specific portability symptom is present significantly improves the
chances of successful LLM repair (50--77\% vs. 27--43\% for generic prompts).

\subsection{Native Portability Remains Essential Despite Containers}

\noindent\textbf{Observation.}~Only 19.9\% of projects in our dataset include
Docker or container configurations, meaning the vast majority (80.1\%) must
handle portability natively in their code. Even projects targeting containerized
production often run development and testing on host operating systems, where
portability issues still manifest.

\noindent\textbf{Recommendation.}~When writing cross-platform Python code,
prefer built-in portable abstractions (\eg{}, \texttt{pathlib} over hardcoded
separators, \texttt{tempfile} over fixed paths) and use our four fix patterns
(Defensive checks, Environment handling, Normalization, Portable APIs) to
address the portability sub-categories in Table~\ref{tab:symptoms}. Maintain
cross-OS CI test matrices to catch issues early, regardless of deployment target.

\subsection{Portability Fixes Are Localized, Not Architectural}

\noindent\textbf{Observation.}~Portability fixes in our study were typically
small, localized changes rather than architectural rewrites. The four fix
patterns we identified (Defensive checks, Environment handling, Normalization,
Portable APIs) address issues through targeted modifications—adding conditional
imports, normalizing paths, or wrapping platform-specific calls—rather than
redesigning system architecture.

\noindent\textbf{Recommendation.}~When encountering a portability issue,
consider it a localized problem rather than deferring it as ``too invasive.''
Consult Table~\ref{tab:symptoms} to identify the sub-category and the
corresponding fix pattern that has been effective for similar issues. Most
solutions involve adding guards or normalization rather than rewriting APIs,
making incremental fixes practical.

\subsection{File and Path Issues Dominate Cross-Platform Failures}

\noindent\textbf{Observation.}~File and Directories is the most prevalent
portability category, affecting 62 projects (41\% of all portability issues).
The most common sub-categories are path separators (34 projects), encoding
(9 projects), and file locking (11 projects), reflecting fundamental
differences in how operating systems handle files and paths.

\noindent\textbf{Recommendation.}~Prioritize file and path portability in code
reviews and when writing new code. Use \texttt{pathlib.Path} for all path
construction instead of string concatenation or hardcoded separators, explicitly
specify \texttt{encoding="utf-8"} when opening text files, and use context
managers (\texttt{with} statements) for file operations to ensure proper closure
and avoid Windows file locking issues. These simple practices prevent the most
common category of portability failures.
...
\noindent
\textbf{Static Analysis for Python.}~Pysa~\cite{pysa2021}, developed by Meta, is a static analysis framework for Python that performs data-flow tracking to identify potential security vulnerabilities. While Pysa demonstrates the effectiveness of static analysis for Python codebases, it focuses exclusively on security issues (taint analysis, SQL injection, XSS) rather than portability concerns. 
In contrast, our work addresses cross-platform compatibility issues that manifest at runtime due to OS differences. Moreover, our evaluation shows that existing static analysis tools (including security-focused ones like Pysa) provide minimal support for detecting the portability issues we identify.
...
\noindent
\textbf{Portability Measurement and Assessment.}~Ghandorh \etal{}~\cite{ghandorh2020systematic} performed a systematic literature review of metrics and methodologies for assessing software portability. 
...
\noindent
\textbf{OS Variability and System Reliability.}~Sun \etal{}~\cite{sun2016bear} proposed \textit{Bear}, a framework that statistically measures application sensitivity to operating system (OS) variability. 
By analyzing how nondeterministic OS behaviors affect reliability and performance, they demonstrate that assumptions about OS predictability can lead to unforeseen failures. 
While their work focuses on OS-level nondeterminism and statistical reliability analysis, our study addresses deterministic portability issues that arise from platform-specific APIs, file system differences, and library dependencies. 
Additionally, Bear requires statistical modeling of OS behavior variability, whereas our approach directly identifies concrete portability problems through cross-platform testing and provides actionable fix patterns for developers.

\Fix{Others}: \cite{sun2016bear}, Mobile apps testing: \cite{boushehrinejadmoradi2015testing}

\section{Conclusions and Future Work}
\label{sec:conclusions}

\Den{Ali, please check the numbers below.}
This study provides the first comprehensive empirical characterization of portability issues in Python code, demonstrating that cross-platform failures are both prevalent and systematically addressable. Through analysis using cross-OS test re-execution and issue mining, we found that 25.7\% of projects exhibit portability problems, with File and Directories operations being the dominant failure category (62 projects). 
Our taxonomy identifies 7 root-cause categories with 24 sub-categories, revealing that 62.5\% of portability issues produce unique error signatures that enable immediate diagnosis, while the remainder require contextual analysis. We discovered 4 general fix patterns—Defensive checks, Environment handling, Normalization, and Portable APIs—that collectively address all observed portability problems through localized code modifications rather than architectural changes. 
Our evaluation shows that existing static analysis tools provide minimal portability support, while LLMs achieve moderate detection accuracy (65-79\%) and substantially improved repair success (50-77\%) when guided by structured symptom-pattern mappings. 
The practical value of our findings is validated through 33 submitted pull requests with a 48\% acceptance rate and zero rejections.

Future work should explore several promising directions to advance automated portability support. First, developing hybrid detection tools that combine static analysis for context-dependent symptoms with dynamic cross-platform testing could provide more comprehensive coverage than either approach alone. 
Second, investigating fine-tuned or specialized LLMs trained specifically on portability patterns could improve both detection accuracy and repair success rates beyond current general-purpose models. 
Third, extending our taxonomy to other cross-platform languages (e.g., JavaScript, Java, C++) would enable comparative analysis and identification of language-agnostic portability principles. 

Finally, longitudinal studies tracking how portability issues evolve with language and ecosystem changes could inform proactive development practices. The systematic understanding established by this work provides a foundation for building more robust cross-platform development tools and practices that can significantly reduce the burden of portability maintenance in modern software development.

\section*{Data Availability}
\Fix{I dont know if we need this section.}
The artifacts -- including datasets and scripts -- are publicly available~\ourrepo{}.
