...
Following the classification experiment, we conducted a complementary evaluation focused on the corrective capabilities of LLMs. In this setting, we used the \NumCodeNonPortableByLLM{} non-portable Python snippets identified earlier and prompted each model with a generic instruction explicitly stating that the code exhibited portability issues and requesting a corrected version. The goal was to assess whether the models could not only recognize but also repair cross-platform defects by producing functionally portable code. We then manually verified whether the generated fixes effectively resolved the underlying portability problem without altering the intended semantics of the original program.

Each model thus attempted to repair \NumCodeNonPortableByLLM{} faulty code samples, yielding a total of 90 attempted fixes across all LLMs. We evaluated the correctness of each fix based on two criteria: (i) the modified code should run without errors on all target platforms (Linux, macOS, Windows), and (ii) it should preserve the original functionality as closely as possible. Fixes that met both criteria were classified as correct; otherwise, they were deemed incorrect.
...
\begin{table}[h!]
\centering
\caption{Performance of LLMs in generating correct portability fixes across both prompt configurations (\NumCodeNonPortableByLLM{} samples per model).}
...
The inclusion of pattern-guided information significantly improved repair accuracy across all evaluated models. As shown in Table~\ref{tab:llm_fix_results_combined}, the overall success rate increased markedly compared to the generic prompt setting. The \texttt{grok-4-fast} model achieved the highest accuracy at 76.67\%, successfully correcting 23 out of \NumCodeNonPortableByLLM{} faulty snippets. This improvement of more than \NumCodeNonPortableByLLM{} percentage points relative to the previous experiment indicates that the model can effectively leverage structured repair cues to guide its reasoning and produce more consistent, portable fixes.

The \texttt{gpt-4o-mini} model also benefited substantially from the pattern-guided prompts, reaching an accuracy of 70.00\%. Its fixes were generally more complete and aligned with the provided repair strategies, demonstrating a clearer understanding of portability concepts when supplied with contextual examples. However, some remaining errors suggest occasional overfitting to the pattern description—where the model followed the suggested fix mechanically without fully adapting it to the specific snippet structure.

The \texttt{llama-3.3} model showed moderate improvement, achieving an accuracy of 50.00\%. While still the weakest performer, it displayed greater consistency compared to the generic prompt scenario. The model was able to apply straightforward fix patterns, such as replacing hard-coded paths or handling platform-dependent APIs, but often failed to integrate the repair logic correctly within more complex code contexts. Overall, these results demonstrate that incorporating explicit repair patterns can substantially enhance the effectiveness of LLM-based portability fixes, although the extent of improvement varies across models.
