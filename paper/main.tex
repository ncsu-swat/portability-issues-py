...
analyzed through systematic mining of \github{} issues and pull requests 
(both methods described in Section~\ref{sec:methods}). These two subsets 
...
requests that address identified portability issues (discussed in 
Section~\ref{sec:pr-discussion}).
...
first method, test re-execution, systematically reveals behavioral 
differences across platforms but is inherently incomplete as tests may 
not cover important code paths. The second method, issue mining, 
...
\TotalProjectsWithPortabilityIssues{} Python projects with portability issues 
across both methods.
...
is essential for rapid diagnosis and triage. In this section, we examine 
...
human judgment and code inspection. This suggests that fully automated portability 
detection tools must go beyond simple log parsing and incorporate contextual analysis, 
such as static code analysis to detect hardcoded paths, shell command patterns, or 
...
\textbf{(i) Defensive checks} add guards, fallbacks, or conditional imports to handle 
...
\textbf{(ii) Environment handling} adapts code and tests to runtime conditions by 
...
\textbf{(iii) Normalization} standardizes encodings, line endings, paths, or filenames 
...
\textbf{(iv) Portable APIs} replace OS-specific constructs with cross-platform 
...
\MyPara{Pattern distribution}~Figure~\ref{fig:fix_pattern_distribution} shows how 
these patterns distribute across the 24 portability sub-categories. Defensive checks 
and Environment handling are the most prevalent strategies, each addressing 9 
sub-categories (33.3\%), together accounting for two-thirds of all portability fixes. 
Normalization applies to 6 sub-categories (22.2\%), primarily addressing file and 
path-related issues. Portable APIs, while used least frequently (3 sub-categories, 
11.1\%), targets specific, well-defined problems where standard cross-platform 
abstractions exist. This distribution reflects the nature of portability issues: 
most problems arise from platform-specific features or environmental differences 
requiring conditional handling, rather than fundamental API incompatibilities. 
The examples demonstrate that portability fixes are typically small, localized 
changes rather than major refactoring efforts, making them practical for developers 
to adopt.
