...
% 
The problem falls into the \textbf{API Availability (API)} category, specifically the ``Methods: os.uname(), os.geteuid(), os.getuid(), os.getpgid()'' sub-category. The issue occurred where \CodeIn{os.geteuid()} was used to check for root privileges, but this function is unavailable on Windows, causing \texttt{AttributeError: module `os' has no attribute `geteuid'}.
% 
...
Our fix creates a cross-platform \CodeIn{is\_admin()} function that uses Windows API for administrator checks on Windows and \CodeIn{os.geteuid()} on Unix-like systems. This exemplifies the \textbf{Portable APIs + Defensive checks} fix strategy, where platform-specific APIs are wrapped with conditional logic and appropriate fallbacks.
