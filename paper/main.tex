...
(now Apptainer)~\cite{apptainer} were introduced to address software
portability challenges by encapsulating code, dependencies, and
execution environments into reproducible units. This approach
minimizes configuration discrepancies across platforms and facilitates
consistent testing and deployment. Nevertheless, despite the clear
benefits of containerization, most open-source Python projects remain
non-containerized. In our analysis of \NumProjectsTotal{}
\github\ repositories, only 19.9\% included a Dockerfile or Docker
...
detect issues, with LLMs achieving 65--79\% accuracy in detection and 50--77\% success in repair when provided with structured guidance.
We conclude with a set of lessons we learn.
...
Our artifacts, including code and data, is publicly available at \ourrepo{}.
...
\begin{packed_enumerate}
\item \textbf{Native Portability Remains Essential Despite Containers}
...
\item \textbf{Guided LLM Repair Using Error Signatures and Fix Patterns}

\noindent\textbf{Observation.}~LLMs underperform with generic ``fix this code'' prompts
(27--43\% success) but become more effective when guided by structure: including the
...
\noindent\textbf{Recommendation.}~When using LLMs to fix portability issues,
include the specific error message and suggest the corresponding fix pattern
from Table~\ref{tab:symptoms} in your prompt. This structured approach
nearly doubles success rates (50--77\%) compared to generic ``fix this code''
prompts (27--43\%). Future code generation tools (e.g., GitHub Copilot)
could prevent these issues by incorporating portability awareness during code authoring.



\item \textbf{Portability Fixes Are Localized, Not Architectural}

\noindent\textbf{Observation.}~Portability fixes in our study were typically
small and localized. The four fix
patterns we identified (Table~\ref{tab:symptoms}) address issues through targeted modifications—adding conditional
imports, normalizing paths, or wrapping platform-specific calls—rather than
redesigning system architecture.

\noindent\textbf{Recommendation.}~Treat portability issues as localized problems
that can be fixed with small, targeted changes. Consult Table~\ref{tab:symptoms}
to identify the issue type and apply the corresponding fix pattern rather than
deferring the fix as ``too invasive.'' Most solutions require only adding
guards, normalizing paths, or using portable APIs.

\item \textbf{File and Path Issues Dominate Cross-Platform Failures}
...
\noindent\textbf{Internal validity:} Our cross-OS test re-execution relies on GitHub Actions runners, which may differ from physical machines. 
Manual GitHub issue analysis involved subjective interpretation, mitigated through iterative coding by multiple authors.

\noindent\textbf{External validity:} Our \NumProjectsTotal{} GitHub projects may not represent private repositories, enterprise software, or domain-specific applications. 
Results may not generalize to legacy systems or embedded applications, though our taxonomy emerges from fundamental OS differences.

\noindent\textbf{Construct validity:} Our ``portability issues'' definition focuses on behavioral differences across Linux, macOS, and Windows, excluding hardware architectures, Python versions, or dependency variations. 
The taxonomy remains subjective despite systematic comparison across data sources and community validation.

\noindent\textbf{Conclusion validity:} Sample sizes vary across research questions (\NumProjectsReExecution{}, \NumProjectsIssueMining{}, \NumCodeUsedLLM{} snippets), affecting comparison reliability. 
The 48\% pull request acceptance rate provides validation, but \TotalPROpened{} samples limit statistical confidence. Results may not generalize beyond moderate-sized LLMs or English-language discussions.
...
\noindent
\textbf{Empirical Studies on portability.}~Ghandorh \etal{}~\cite{ghandorh2020systematic} performed a systematic literature review of metrics and methodologies for assessing software portability. 
...
This study provides the first comprehensive empirical characterization of portability issues in Python code. Through cross-OS test re-execution and issue mining, we found that 25.7\% of projects exhibit portability problems, with File and Directories operations being most prevalent. 
Our taxonomy identifies 7 root-cause categories with 24 sub-categories and 4 general fix patterns. LLMs achieve 65-79\% detection accuracy and 50-77\% repair success when guided by structured patterns. Our findings are validated through 33 pull requests with 48\% acceptance rate.

Future work should develop hybrid detection tools and investigate specialized LLMs for portability patterns.
