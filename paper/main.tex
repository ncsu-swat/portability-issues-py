...
Across projects, we observe that fixes consistently reduce to four general patterns: (i) \textbf{Defensive checks}, which add guards, fallbacks, \texttt{try/except}, or conditional imports to avoid crashes when APIs or modules are missing; (ii) \textbf{Portable APIs}, which replace OS-specific constructs with cross-platform abstractions (e.g., \texttt{pathlib}, \texttt{platform}, \texttt{psutil}) and safer subprocess usage; (iii) \textbf{Normalization}, which standardizes encodings, line endings, paths, or filenames before comparison to prevent spurious differences; and (iv) \textbf{Environment handling}, which adapts code and tests to runtime conditions (e.g., skipping GUI in headless CI, handling permissions/dependencies, or using containers/virtual environments). These patterns are simple, teachable, and cover the majority of fixes we observed.

\textbf{Concrete example (PR fix).} [Placeholder: We will insert a specific PR and code diff illustrating the applied fix.]
