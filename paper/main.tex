...
\noindent\textbf{Internal validity:} Our cross-OS test re-execution relies on GitHub Actions runners, which may differ from physical machines. Manual GitHub issue analysis involved subjective interpretation. \emph{Mitigation:} We used standardized GitHub runners and employed iterative coding by multiple authors.

\noindent\textbf{External validity:} Our \NumProjectsTotal{} GitHub projects may not represent private repositories, enterprise software, or domain-specific applications. \emph{Mitigation:} We sampled diverse projects across different domains and our taxonomy emerges from fundamental OS differences.

\noindent\textbf{Construct validity:} Our ``portability issues'' definition focuses on behavioral differences across Linux, macOS, and Windows, excluding hardware architectures, Python versions, or dependency variations. \emph{Mitigation:} We validated our taxonomy through systematic comparison across two independent data sources and community validation via pull requests.

\noindent\textbf{Conclusion validity:} Sample sizes vary across research questions (\NumProjectsReExecution{}, \NumProjectsIssueMining{}, \NumCodeUsedLLM{} snippets), affecting comparison reliability. \emph{Mitigation:} We triangulated findings across multiple methods and achieved \TotalPRAprovedPorcent{} pull request acceptance rate.
