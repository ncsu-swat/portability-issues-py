...
\begin{abstract}
  \sloppy
While Python is designed as a cross-platform language, real-world applications encounter portability failures when deployed across different operating systems. We present the first large-scale empirical study of portability issues in Python, analyzing \NumProjectsTotal{} open-source repositories using two complementary approaches: systematic cross-OS test re-execution and manual analysis of GitHub issues. Our cross-platform testing of \NumProjectsReExecution{} projects reveals that \PercentProjectsWithPortabilityReExecution{} exhibit OS-dependent test failures, with \NumTestRunsWithOSDiff{} divergent outcomes across \NumTestsExecuted{} test executions. Through systematic analysis of \NumIssuesAnalyzed{} GitHub issues, we confirm \NumIssuesRelated{} genuine portability problems spanning \NumProjectsWithOSDiffInIssues{} additional projects. We develop a comprehensive taxonomy identifying \NumRootCauseCategories{} primary failure categories—with file/directory operations, process management, and library dependencies being most prevalent—along with \NumRootCauseSubcategories{} distinct sub-categories, \NumSymptomSignatures{} diagnostic signatures, and \NumFixPatterns{} systematic repair patterns. Our evaluation reveals that existing static analysis tools provide minimal support for portability detection, while large language models achieve 65--79\% accuracy in identifying issues and 50--77\% success in generating fixes when provided with structured guidance. Through \TotalPROpened{} contributed pull requests, we demonstrate practical applicability and developer acceptance (\TotalPRAccepted{} merged, zero rejected) of our findings. This work establishes the first comprehensive baseline for understanding and addressing Python portability issues, providing actionable insights for developers, tool designers, and the broader research community. 
\end{abstract}
...
most frequent (18 PRs), reflecting the prevalence of path, encoding,
...
This section synthesizes key insights from our comprehensive analysis of portability issues in Python, offering actionable guidance for developers, tool designers, and researchers working on cross-platform software systems.

\subsection{Lesson 1}

Our taxonomy reveals that portability failures are \textit{not} random or project-specific anomalies, but instead cluster around a small set of well-defined categories. The 7 root-cause categories and 24 sub-categories we identified capture the vast majority of cross-platform issues encountered in real-world Python projects. More importantly, 62.5\% of these issues produce unique error signatures that enable immediate diagnosis from logs alone.

\textbf{For developers:} Focus defensive coding efforts on the most prevalent categories: file operations (62 projects affected), process management (25 projects), and library dependencies (24 projects). Use our symptom signatures (Table~\ref{tab:symptoms}) as diagnostic checklists when triaging cross-platform failures.

\textbf{For researchers:} Portability research should shift from ad-hoc case studies toward systematic approaches based on our established categories. Future work can develop domain-specific taxonomies or investigate emerging categories as the Python ecosystem evolves.

\subsection{Lesson 2: Cross-Platform Testing Requires High Test Coverage.}

Our dual-method approach revealed that cross-platform testing is only effective with comprehensive test coverage, as test re-execution can only detect issues in exercised code paths. While test re-execution identified systematic behavioral differences, it missed issues in untested areas. Conversely, GitHub issue mining captured edge cases and developer discussions that tests don't exercise. The 48\% acceptance rate of our pull requests demonstrates that both approaches identify genuine problems.

\textbf{For developers:} Cross-platform testing effectiveness is directly limited by test coverage. Prioritize high coverage for file operations, subprocess calls, and platform-specific functionality before running cross-OS tests. 

\textbf{For researchers:} Comprehensive portability assessment requires multi-faceted approaches. Future studies should combine technical analysis (code, tests, logs).

\subsection{Lesson 3: Prioritize Critical Categories for Maximum Impact}
