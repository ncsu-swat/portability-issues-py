...
abstractions. Listing~\ref{lst:portable} demonstrates replacing a hardcoded Unix 
path \texttt{"/tmp/data.txt"} with portable abstractions (\texttt{tempfile.gettempdir()} 
and \texttt{pathlib.Path}), ensuring the code works correctly regardless of the 
operating system's directory structure.
...
% \\
This code has \textit{<root-cause-subcategory>}.  Consider using one of the following fixes \textit{<list-of-possible-fixes-for-the-root-cause>}. \\
% \\
...
The rationale for this approach is: (1) observe root-causes in the code,
(2) look up the mapping in Table~\ref{tab:symptoms} 
(Root-cause $\rightarrow$ General Fix Pattern), and (3) prompt
the LLM with specific fix pattern guidance. For example, when 
ncountering code with \textit{Missing signals} issues (platform-specific 
signal constants), we would prompt: ``This code has platform-specific 
signal handling issues. Consider using the \texttt{Defensive checks} 
...
We run the cross-OS test re-execution method
(\textsection~\ref{sec:methods}) to detect portability problems on our
application set (\textsection~\ref{sec:projects}). After finding
problems, we employ one of the four fix patterns this work finds
(Table~\ref{tab:symptoms}), test the fix locally, and elaborate a pull
request including a detailed explanation of the portability problem
and the proposed fix.\Mar{I revised this. Please check.}
...
\label{sec:lessons}
\Fix{Update the section to use the observation and the recommendations.}
...
\subsection{LLMs Are Effective Portability Assistants When Given Structure}

Our evaluation of large language models reveals a striking pattern: while LLMs 
struggle with generic ``fix this code'' prompts (27--43\% success rate), they 
become effective when provided with structured guidance. By supplying 
the specific portability issue category (\eg{}, ``File and Directories - Encoding'') 
and the corresponding fix pattern (\eg{}, ``Normalization - set explicit UTF-8 
encoding''), repair success rates jumped to 50--77\%, with the best model 
(Grok-4-Fast) achieving 77\% accuracy.

This suggests a practical workflow for developers: (1)~when encountering a 
portability issue, first classify it using our taxonomy (Table~\ref{tab:categories} 
and Table~\ref{tab:symptoms}); (2)~then prompt the LLM with both the problematic 
code \emph{and} the appropriate fix pattern. For example: ``This code has a 
Windows file locking issue. Consider using the \texttt{Environment handling} 
pattern: \texttt{use context managers and explicit close}.''

These results suggest that providing taxonomic context significantly improves 
LLM repair capabilities. This approach may be particularly useful when addressing 
portability issues flagged by CI/CD pipelines or issue reports, where the issue 
category can be identified from error logs before invoking the LLM.

\subsection{Containerization Is Underutilized—Most Projects Need Native Portability}

Container technologies such as Docker and Singularity/Apptainer are effective 
tools for mitigating portability issues by standardizing deployment environments. 
However, our analysis reveals that only 19.9\% of the \NumProjectsTotal{} Python 
projects we examined include \CodeIn{Dockerfile} or Docker Compose configurations. This 
surprisingly low adoption rate—far below what we expected—means that the vast 
majority of Python projects (80.1\%) must handle portability concerns natively 
in their code.

Even when targeting containerized production environments, developers sometimes 
work directly on host systems during development and testing, where portability 
issues manifest. Additionally, many deployment contexts (\eg{}, serverless platforms, 
embedded systems, CI/CD environments, and companies with container 
restrictions) require code to run portably without containerization.

Given this low adoption rate, native code portability remains important. Our 
study identifies four systematic fix patterns—defensive checks, environment 
handling, normalization, and portable APIs—that collectively address all 
\NumRootCauseSubcategories{} portability sub-categories we encountered. The 
prevalence of these patterns across our dataset suggests they represent common 
solutions to recurring portability challenges in Python development.

\subsection{Most Portability Issues Produce Unique Error Signatures}

Our symptom analysis (Table~\ref{tab:symptoms}) shows that the majority of 
sub-categories (15 out of 24, 62.5\%) produce unique, actionable error 
signatures that enable immediate diagnosis from logs alone. Examples include 
\texttt{AttributeError: module `os' has no attribute `geteuid'}, 
\texttt{ModuleNotFoundError: No module named `fcntl'}, 
\texttt{UnicodeDecodeError: `charmap' codec...}, and 
\texttt{OSError: [Errno 24] Too many open files}. These signatures facilitate 
direct diagnosis without extensive code inspection.

This has two practical implications: First, developers can build simple detection 
tools using grep patterns to scan CI/CD logs for known portability signatures. 
Second, cross-platform CI/CD pipelines are highly effective at catching these 
issues, as most failures produce immediate and unambiguous error messages.

The remaining 9 sub-categories (37.5\%) exhibit context-dependent symptoms 
requiring code inspection to diagnose. These include hardcoded path separators 
that fail silently, shell command execution differences, and GUI behavioral 
variations. For these cases, code review practices that specifically check for 
platform-specific constructs become essential, along with cross-platform testing 
to reveal behavioral differences.

\subsection{Test Suites Require the Same Portability Scrutiny as Application Code}

Our pull request analysis reveals an important distribution: the code was 
modified in \TotalPRInTestsCode{} cases (51.5\%) to the test code only, in 
\TotalPRInProgramCode{} cases (30.3\%) to the program code only, and in 
\TotalPRInBothCode{} cases (18.2\%) to both test and program code. This 
distribution suggests that while some portability issues can be resolved by 
adjusting tests to be more environment-aware, many require changes to the 
application logic itself to ensure consistent behavior across platforms.

The high proportion of test-only fixes (more than half) indicates that test 
suites frequently contain platform-specific assumptions that don't exist in the 
application code. Common issues include hardcoded paths in test fixtures, 
assumptions about filesystem behavior (such as file locking or case sensitivity), 
and environment-dependent test conditions (display servers, signal availability).

Developers should apply the same portability scrutiny to test code as they do 
to production code. This includes using platform detection decorators (\eg{}, 
pytest's \texttt{@pytest.mark.skipif}), employing portable abstractions like 
\texttt{tempfile} and \texttt{pathlib} in test fixtures, and avoiding OS-specific 
assertions. Cross-platform CI execution is essential, as test failures often 
reveal platform assumptions that remain invisible during development on a single 
system.
