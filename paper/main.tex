...
such as static code analysis to detect portability ``bugs'' (\eg{}, hardcoded paths, shell command patterns, or 
platform-specific API usage)\Mar{please, confirm}. The presence of context-dependent symptoms also highlights 
...
\begin{subtable}[t]{\textwidth}
\centering
\caption{Examples of pattern-guided prompts used for specific portability issues.}
\label{tab:specific_prompts}
\begin{tabular}{p{0.9\textwidth}}
\toprule
\textbf{Pattern-Guided Prompt Examples} \\
\midrule
\textbf{Example 1 (Shell command execution):} \\
You are a Python expert. Check the following code and answer: \\
This code has a shell command execution issue (PROC category). Apply the Portable APIs pattern to use subprocess.run with shell=False. \\
\\
\textbf{Example 2 (Missing OS methods):} \\
You are a Python expert. Check the following code and answer: \\
This code has a missing OS method issue like os.geteuid() (API category). Apply the Portable APIs + Defensive checks pattern to use platform.uname and hasattr guards. \\
\\
\textbf{Example 3 (GUI differences):} \\
You are a Python expert. Check the following code and answer: \\
This code has a GUI differences issue (ENV category). Apply the Environment handling pattern to skip or mock GUI features. \\
\\
\textbf{Example 4 (File locking):} \\
You are a Python expert. Check the following code and answer: \\
This code has a Windows file locking issue (FILE category). Apply the Environment handling pattern to use context managers and explicit close. \\
\\
\textbf{Example 5 (Missing signals):} \\
You are a Python expert. Check the following code and answer: \\
This code has a missing signals issue (PROC category). Apply the Defensive checks pattern to guard signals per platform. \\
\bottomrule
\end{tabular}
\end{subtable}
\end{table*}
...
In the second configuration (\textit{pattern-guided prompt}), each non-portable snippet was accompanied by a description of the specific portability issue (derived from our taxonomy in \ref{rq2-1}) and the corresponding \textit{General Fix Pattern} from Table~\ref{tab:symptoms}. Table~\ref{tab:llm_prompts} shows examples of how these pattern-guided prompts were structured to provide specific guidance for different portability issues.  
...
submitted a total of \TotalPROpened{} pull requests (PRs) spanning 6
...
repository maintainers. Notably, ``File and Directories'' issues were the
...
\Fix{add a Reference to your PR list in the artifact? \ourrepo{}}
\Fix{but we used your personal account to create the PRs.}
...
We encountered a different type of response when addressing a dependency
 portability issue in the frequenz-sdk-python project~\cite{frequenz_sdk_python}.
  Frequenz-sdk-python is an energy management SDK for Python applications.
   The problem we identified falls into the \textbf{System Information (SYS)} category
    from Table~\ref{tab:categories}, specifically the ``Timezone database'' sub-category.

The issue occurred in the test suite where tests relied on the \CodeIn{zoneinfo}
 and \CodeIn{tzdata} modules for timezone operations. These modules are not
  consistently available across all platforms—while \CodeIn{zoneinfo} is included
   in Python 3.9+ on most systems, the timezone data (\CodeIn{tzdata}) may not be
    installed by default on some Windows configurations. This caused test failures
     when the required timezone information was unavailable, manifesting as
      \texttt{zoneinfo.\_common.ZoneInfoNotFoundError} (see Table~\ref{tab:symptoms}).
...
However, during the PR discussion, the maintainer took a different perspective.
 Rather than adding platform-specific complexity to accommodate the dependency,
  they chose to remove the timezone-dependent tests altogether—eliminating the 
  portability issue by removing the feature that caused it. While this falls 
  outside our documented fix patterns, it represents a pragmatic trade-off: 
  prioritizing long-term maintainability and code simplicity over comprehensive
...
Windows file locking issue (FILE category). Apply the Environment handling 
pattern to use context managers and explicit close.''
...
issues manifest. Additionally, many deployment contexts (serverless platforms, 
embedded systems, CI/CD environments, educational settings, and enterprises with 
container restrictions) require code to run portably without containerization.
