...
portability issues. Table~\ref{tab:dataset-b} shows the results from 
...
manual inspection. Through careful analysis of issue titles, descriptions, 
...
comprehensive test coverage. Importantly, the \NumIssuesRelated{} 
...
organized by root-cause categories. Each row groups related sub-categories
...
\noindent\textbf{File and Directories (FILE):} The most prevalent category (60 projects) stems from OS differences in file operations. Key issues include path separator differences (backslash vs. forward slash), line ending mismatches (CRLF vs. LF), file locking semantics, and text encoding defaults.

\noindent\textbf{Process and Signals (PROC):} This category (25 projects) captures failures from OS-level process management differences. Shell execution varies across platforms (cmd.exe vs. bash), Windows lacks many Unix signals (SIGHUP, SIGKILL), and port binding produces different error codes.

\noindent\textbf{Library Dependencies (LIB):} These failures (24 projects) occur when code assumes platform-specific libraries. Issues include Unix-only modules (e.g., fcntl), missing dependencies, different dynamic library extensions (.dll vs .so vs .dylib), and architecture-specific binary wheels.
...
\noindent\textbf{API Availability (API):} This category (8 projects) covers Python APIs not universally available. Many os module methods are Unix-specific (os.uname(), os.geteuid(), os.getpgid()) and fail on Windows, while optional modules like readline and resource may be missing.
...
\noindent Table~\ref{tab:symptoms} presents diagnostic symptom signatures mapped to portability sub-categories. Each row represents a specific sub-category within a broader root-cause category (FILE, PROC, LIB, ENV, API, PERM, SYS). The columns show: (i) the root-cause category, (ii) the specific sub-category, (iii) verbatim error messages or symptoms when available, and (iv) the recommended general fix pattern. Not every sub-category produces a unique diagnostic signature; where concrete error messages exist, they reliably identify the underlying cause in test/CI logs (e.g., \texttt{AttributeError: module 'os' has no attribute 'uname'}, \texttt{UnicodeDecodeError}). These signatures enable rapid triage and help developers quickly recognize portability issues in practice.
...
\midrule
\texttt{llama-3.3-70b-instruct} & 0.714 & 0.167 & 0.270 & 0.400 \\
...
% \begin{table}[h!]
% \centering
% \caption{\label{tab:confusion_matrix_llama}Confusion matrix -- \texttt{llama-3.3-70b-instruct}}
% \begin{tabular}{lcc}
% \toprule
...
\toprule
& \multicolumn{3}{c|}{\texttt{llama-3.3-70b-instruct}} & \multicolumn{3}{c|}{\texttt{grok-4-fast}} & \multicolumn{3}{c}{\texttt{gpt-4o-mini}} \\
...
\begin{enumerate}
  \item \texttt{Llama-3.3-70B-Instruct}: Conservative and shallow reasoning: prioritizes safety but lacks sensitivity, leading to under-detection.
