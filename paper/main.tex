...
\Ali{Please review updates: (smaller paragraphs, remove the 100 percentage accuracy of re-execution method, add example of issue mining findings, ...)}
...
we mined a total of \NumProjects{} projects but re-executed test suites of
nearly 25\% of them, \ie{}, \NumProjectsAnalyzed{} projects, due to the 
resource-intensive nature of cross-platform test execution across three operating systems. Recall
...
corresponds to \PercentDiscrepantTests{}. The test re-execution
method provides a systematic and reliable approach to detecting
portability issues by directly observing behavioral differences across
platforms.

In contrast, the issue mining method relies on text analysis
of developer discussions and issue reports. Due to the inherent
variability of natural language and the need to interpret context, we
manually analyze the discussion threads of the issues to confirm
whether they indicate real portability issues. 
...
42.5\%. While this method requires significant manual effort to validate
candidates, it provides complementary insights 
...
comprehensive test coverage. For instance, we found an issue where a 
project's tutorial failed on Windows due to a platform-incompatible 
dependency, prompting maintainer discussions about whether to make 
dependencies optional or create platform-specific variants—decision-making 
processes and untested code paths that test re-execution alone cannot capture~\cite{pyearthtools-issue82}. 
...
\Ali{Please review updates: (methodology explained at the beginning of the section,color-coded fix patterns, ...)}

To answer RQ2, we performed qualitative analysis on the 
portability issues identified through test re-execution (\NumTestRunsWithOSDiff{} 
test differences) and issue mining (\NumIssuesRelated{} confirmed issues). For 
each instance, we manually inspected error messages, stack traces, failing test 
code, and—in the case of \github{} issues—developer discussions. Through iterative 
open coding, we identified recurring patterns and grouped issues with similar 
underlying causes. This process was conducted by the authors, with 
regular discussions to resolve ambiguities and refine category definitions. The 
resulting taxonomy consists of 7 high-level root-cause categories (e.g., FILE, 
PROC, LIB) with 24 distinct subcategories. For each issue, we also documented 
observable symptoms (error messages, behaviors) and examined corresponding fixes 
in merged pull requests or commits to identify common repair patterns.
...
\noindent\textbf{File and Directories (FILE):} The most prevalent category (60 projects) stems
...
\noindent\textbf{Process and Signals (PROC):} This category (25 projects) captures failures
...
\noindent\textbf{Library Dependencies (LIB):} These failures (24 projects) occur when code
...
\noindent\textbf{Environment and Display (ENV):} These problems (14 projects) 
...
\noindent\textbf{API Availability (API):} This category (8 projects) covers Python APIs
...
\noindent\textbf{Permissions and Limits (PERM):} These issues (7 projects) reflect OS
...
\noindent\textbf{System Information (SYS):} This category (2 projects) includes failures
...
\begin{table*}[h!]
\centering
\caption{\label{tab:confusion_matrices_combined}Confusion matrices showing classification performance of three LLMs on the portability detection task. Rows represent actual labels, columns represent predicted labels, and diagonal entries indicate correct classifications.}
...
% 
Their performance highlights a key limitation of current LLMs in software reasoning: a tendency to rely on surface-level heuristics rather than deep contextual understanding of cross-platform behavior. Addressing this limitation will require more explicit exposure to platform-diverse examples during instruction tuning and evaluation.
...
Each model thus attempted to repair 30 faulty code samples, yielding a total of 90 attempted fixes across all LLMs. We evaluated the correctness of each fix based on two criteria: (i) the modified code should run without errors on all target platforms (Linux, macOS, Windows), and (ii) it should preserve the original functionality as closely as possible. Fixes that met both criteria were classified as correct; otherwise, they were deemed incorrect.


We divided this experiment into two complementary settings. In the first, we used a generic prompt, simply informing the model that the code contained a portability issue and requesting a fix, without providing any specific guidance. In the second, we augmented the prompt by explicitly describing the type of portability problem observed and including the corresponding General Fix Pattern from Table~\ref{tab:symptoms}. This second setup aimed to evaluate whether giving the models contextual hints and structured repair patterns would improve their ability to generate correct and portable fixes.


\textbf{Generic prompts for portability repair.}
% part 2

Table~\ref{tab:llm_fix_results} summarizes the quantitative results. Overall, out of the 90 non-portable snippets, only 38 were correctly repaired, corresponding to an overall success rate of 42\%. This moderate performance highlights the challenge of producing accurate cross-platform corrections even when the model is explicitly informed that the input code is non-portable.

\begin{table}[h!]
\centering
\caption{Performance of LLMs in generating correct portability fixes when prompted with non-portable Python snippets (30 samples per model).}
...
\textbf{Pattern-guided prompts for portability repair.}
% part 3

To assess whether providing structured guidance could enhance the models' repair capabilities, we conducted a second round of experiments using pattern-guided prompts. In this setup, each non-portable snippet was accompanied by a description of the specific portability issue it exhibited, along with the corresponding General Fix Pattern from Table~\ref{tab:symptoms}. This additional context aimed to focus the models' attention on relevant aspects of the problem and suggest appropriate repair strategies.
...
\begin{table}[h!]
\centering
\caption{Performance of LLMs in generating correct portability fixes with pattern-guided prompts (30 samples per model).}
\label{tab:llm_fix_results_pattern_guided}
\begin{tabular}{lrrr}
\toprule
\textbf{Model} & \textbf{Correct} & \textbf{Incorrect} & \textbf{Accuracy} \\
\midrule
...
The inclusion of pattern-guided information improved repair accuracy across all evaluated models. As shown in Table~\ref{tab:llm_fix_results_pattern_guided}, the overall success rate increased markedly compared to the generic prompt setting. The \texttt{grok-4-fast} model achieved the highest accuracy at 76.67\%, successfully correcting 23 out of 30 faulty snippets. This improvement of more than 30 percentage points relative to the previous experiment indicates that the model can effectively leverage structured repair cues to guide its reasoning and produce more consistent, portable fixes.

The \texttt{gpt-4o-mini} model also benefited substantially from the pattern-guided prompts, reaching an accuracy of 70\%. Its fixes were generally more complete and aligned with the provided repair strategies, demonstrating a clearer understanding of portability concepts when supplied with contextual examples. However, some remaining errors suggest occasional overfitting to the pattern description—where the model followed the suggested fix mechanically without fully adapting it to the specific snippet structure.

The \texttt{llama-3.3} model showed moderate improvement, achieving an accuracy of 50\%. While still the weakest performer, it displayed greater consistency compared to the generic prompt scenario. The model was able to apply straightforward fix patterns, such as replacing hard-coded paths or handling platform-dependent APIs, but often failed to integrate the repair logic correctly within more complex code contexts. 

Overall, these results demonstrate that incorporating explicit repair patterns can substantially enhance the effectiveness of LLM-based portability fixes, although the extent of improvement varies across models.
