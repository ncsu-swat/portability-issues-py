...
\subsection{Selection criteria.} We selected projects based on the following criteria: \Fix{DO IT!}
...
\subsection{Methods}\textbf{Cross-OS re-execution.} For each project we run the test suite on multiple operating systems and record divergent outcomes. We then triage failures via logs and small code inspections, mapping each instance to a concrete category.\Fix{Explain what is a category.}  
...
The macOS setup ran on an Apple M2 Pro chip (Arm64) with 5 CPU cores, 8 GPU cores, 14 GB unified memory, and macOS 15. The Windows setup used an AMD EPYC 7763 64-Core Processor with 4 CPUs, 16 GB RAM, and Windows Server 2025.
...
\subsubsection{RQ2.1 — Root Causes}
\label{rq2-1}
\RQTwoOne{}
...
\begin{table*}[t]
    \centering
    \caption{\Ali{Should we convert this to a single-column table instead, so it only takes half of the page width? } Each row lists a root-cause category with representative sub-categories. Columns report: (i) distinct projects per sub-category observed in mined issues and in test outcomes; and (ii) the overall total (issues + tests).}
    ...
    \toprule
    \textbf{Acronym} & \textbf{Root-cause category} & \textbf{Sub-categories} & \textbf{Sub-category projects (tests)} & \textbf{Sub-category projects (issues)} & \textbf{Total projects} \\
    \midrule
    
    \multirow[t]{6}{*}{\textbf{FILE}} & \multirow[t]{6}{*}{File and Directories} 
    & Path separators & 27 & 7 & \multirow{6}{*}{59} \\
    & & Line endings & 4  & 1 &                      \\
    & & Case sensitivity & 1  & 1 &                      \\
    & & File locking & 4 & 5 &                      \\
    & & Encoding & 7 & 2 &                      \\
    & & Filesystem block size & 1 & 0 &                      \\
    \midrule
    
    \multirow[t]{3}{*}{\textbf{PROC}} & \multirow[t]{3}{*}{Process and Signals}
    & Shell command execution & 14 & 5 & \multirow{3}{*}{25} \\
    & & Missing signals & 2  & 3 &                      \\
    & & Address already in use & 0  & 1 &                      \\
    \midrule
    
    \multirow[t]{4}{*}{\textbf{LIB}} & \multirow[t]{4}{*}{Library Dependencies}
    & Platform-specific libraries (e.g. fcntl) & 1  & 2 & \multirow{4}{*}{24} \\
    & & Missing libraries & 11 & 2 &                      \\
    & & Dynamic library loading & 5  & 0 &                      \\
    & & Binary wheel mismatch & 3 & 0 &                      \\
    \midrule
    
    \multirow[t]{4}{*}{\textbf{ENV}} & \multirow[t]{4}{*}{Environment and Display}
    & No display in github CI (linux) & 2 & 3 & \multirow{4}{*}{14} \\
    & & GUI differences & 4 & 0 &                      \\
    & & Missing curses & 0 & 1 &                      \\
    & & Terminal capabilities & 2 & 2 &                      \\
    \midrule
    
    \multirow[t]{2}{*}{\textbf{API}} & \multirow[t]{2}{*}{API Availability}
    & Methods: os.uname(), os.geteuid(), os.getuid(), os.getpgid() & 2 & 4 & \multirow{2}{*}{8} \\
    & & Modules: readline, resource & 2 & 0 &                      \\
    \midrule
    
    \multirow[t]{3}{*}{\textbf{PERM}} & \multirow[t]{3}{*}{Permissions and Limits}
    & Permission & 1 & 3 & \multirow{3}{*}{7} \\
    & & File descriptor limits & 1 & 0 &                     \\
    & & Symlink privileges & 1 & 1 &                     \\
    \midrule
    
    \multirow[t]{2}{*}{\textbf{SYS}} & \multirow[t]{2}{*}{System Information}
    & /proc filesystem & 0 & 1 & \multirow{2}{*}{2} \\
    & & Timezone database & 0 & 1 &                      \\
    \midrule
    \textbf{} & \textbf{Total} & - & \textbf{95} & \textbf{45} & \textbf{140} \\
    \bottomrule
    \end{tabularx}
\end{table*}
...
\noindent\textbf{File and Directories (FILE).}
The most prevalent category involves file handling differences across operating systems. Issues include path separator mismatches (forward vs. backslash), line ending inconsistencies (CRLF vs. LF), case sensitivity variations in filesystems, file locking behavior (especially on Windows), text encoding problems, and filesystem block size assumptions.

\noindent\textbf{Process and Signals (PROC).}
Cross-platform discrepancies in process execution manifest through shell command execution differences, missing signal handlers (e.g., SIGKILL, SIGHUP unavailable on Windows), and port binding conflicts that produce different error codes across systems.

\noindent\textbf{Library Dependencies (LIB).}
Platform-specific library availability causes failures when code relies on libraries like fcntl (Unix-only), encounters missing dependencies, faces dynamic library loading issues (DLL vs. SO files), or experiences binary wheel architecture mismatches.

\noindent\textbf{Environment and Display (ENV).}
Runtime environment differences emerge when tests require displays in headless CI environments, depend on GUI window managers, attempt to use curses on Windows, or rely on specific terminal capabilities not universally available.

\noindent\textbf{API Availability (API).}
Platform-specific APIs in Python's standard library cause portability issues, particularly missing methods (os.uname(), os.geteuid(), os.getuid(), os.getpgid()) and modules (readline, resource) unavailable on certain platforms.

\noindent\textbf{Permissions and Limits (PERM).}
System permission and resource limit differences include permission errors with varying behavior, file descriptor limits that differ by OS, and symlink privilege restrictions (requiring administrator rights on Windows).

\noindent\textbf{System Information (SYS).}
Platform-specific system information sources cause failures when code assumes /proc filesystem availability (absent on macOS/Windows) or relies on timezone databases not installed by default on all systems.

\textbf{Quick Answer.} Portability issues cluster around 7 main categories: file handling, process execution, library dependencies, environment mismatches, API availability, permissions, and system information sources.
...
\begin{table*}[t]
    ...
    \renewcommand{\arraystretch}{1.15}
    \begin{tabularx}{\textwidth}{
        >{\raggedright\arraybackslash}m{0.05\textwidth}
        >{\raggedright\arraybackslash}m{0.18\textwidth}
        >{\raggedright\arraybackslash}X
        >{\raggedright\arraybackslash}m{0.32\textwidth}
    }
    \toprule
    \textbf{Cause} & \textbf{Sub-categories} & \textbf{Symptom (verbatim error / message)} & \textbf{General Fix Pattern} \\
    \midrule
    \textbf{FILE} &
    ...
    & File locking & \texttt{PermissionError: [Errno 13] Permission denied: 'filename'} & Environment handling — close files retry on Windows \\
    ...
    \midrule
    \textbf{LIB} &
    Platform-specific libraries (e.g. fcntl) & \texttt{ModuleNotFoundError: No module named 'fcntl'} & Defensive checks — conditional import provide fallback \\
    & Missing libraries & \texttt{ModuleNotFoundError: No module named '...'} & Environment handling — conditionally install optional dependency \\
    ...
    \texttt{AttributeError: module 'os' has no attribute 'uname'} (example) & Portable APIs — use platform.uname or defensive checks \\
    ...
    \midrule
    \textbf{PERM} &
    Permission & \texttt{PermissionError: [Errno 13] Permission denied: '...'} & Environment handling — handle permissions document requirements \\
...
\subsubsection{RQ2.2 — Symptoms}
\label{rq2-2}
\RQTwoTwo{}

\subsection{Symptom signatures} 

For subcategories with concrete signals, we list verbatim exceptions or OS-specific error texts that reliably identify the underlying cause in test/CI logs. Not every subcategory has a unique signature; we include only those with specific, reproducible messages (e.g., \texttt{AttributeError: module 'os' has no attribute 'uname'}, \texttt{UnicodeDecodeError: 'utf-8' codec can't decode byte 0x..}). These signatures are meant to speed triage and help developers and tools quickly recognize portability issues; they are not exhaustive of all possible manifestations (see Table~\ref{tab:symptoms}).  

\textbf{Quick Answer.} Most portability issues produce distinctive error signatures that enable quick diagnosis and categorization.

\subsubsection{RQ2.3 — Fix Patterns}
\label{rq2-3}
\textbf{Research Question.} What are effective fix patterns for addressing portability issues?  
\textbf{Rationale.} Identifying reusable solutions helps developers apply proven strategies to resolve portability problems. By analyzing fixes across projects, we can distill common patterns that effectively address cross-platform issues without requiring extensive platform-specific code.
...
Table~\ref{tab:symptoms} maps each root-cause category to its recommended fix patterns, providing a practical guide for addressing specific portability issues:
...
\textbf{Quick Answer.} Four fix patterns (defensive checks, portable APIs, normalization, environment handling) address the majority of portability issues we observed.
...
\label{rq4-2}
\textbf{Research Question.} \RQFourTwo{}
