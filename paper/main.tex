...
\noindent\textbf{Setup.}~To validate our findings and assess how the open-source community responds to portability issues, we submitted pull requests addressing the portability problems that we identified in real-world Python projects. 

We applied our four general fix patterns (defensive checks, environment handling, normalization, and portable APIs) to real-world portability issues found in active Python projects. Each pull request included a detailed explanation of the portability problem and the proposed fix following the corresponding pattern from Table~\ref{tab:symptoms}.

\Den{Add a Reference to your PR list in the artifact? \ourrepo{}}
...
Considering the location of the fix, we observe that the code is
modified in \TotalPRInTestsCode{} cases to the test code only, in
\TotalPRInProgramCode{} cases to the program code only, and in
\TotalPRInBothCode{} cases to both the test and program code. This
distribution suggests that while some portability issues can be
resolved by adjusting tests to be more environment-aware, many require
changes to the application logic itself to ensure consistent behavior
across platforms.
