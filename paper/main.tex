...
The macOS setup ran on an Apple M2 Pro chip (Arm64) with 5 CPU cores, 8 GPU cores, 14 GB RAM, and macOS 15. The Windows setup used an AMD EPYC 7763 64-Core Processor with 4 CPUs, 16 GB RAM, and Windows Server 2025.
...
\begin{table}[t!]
  ...
  \begin{subtable}[t]{\linewidth}
    ...
    \hspace{2ex}Total number of projects with OS differences & \Fix{95??} \\    
    Total number of tests executed & 440,728 \\
    \hspace{2ex}Total number of test runs with OS differences & 9,508 \\
  ...
  \begin{subtable}[t]{\linewidth}
    ...
    \begin{tabular}{lc}
      \toprule
      Total number of issues mined & 2,617 \\
      Total number of issues analyzed & 240 \\
      Total number of issues related & 102\\
      \bottomrule
    \end{tabular}
  \end{subtable}
\end{table}
...
inspecting issues from projects. Table~\ref{tab:dataset-a} shows that
we mined a total of \NumProjects{} but re-executed test suites of
nearly 25\% of them, \ie{}, \NumProjectsAnalyzed{} projects. Recalll
...
compared for discrepancy. The \NumProjectsAnalyzed{} projects whose
test suites were executed included a total of 440,728 test cases. Of
those, we found discrepant outcomes (\ie{}, there was no agreement
across all three OSes that we considered) in 9,508 cases, which
corresponds to 2.16\%\Mar{please, add macros!}. It is worth noting
...
portability issues. Table~\ref{tab:dataset-b}\Fix{...}\Mar{@Ali, please,
explain your table as we do above.}
...
issues are relatively prevalent. For example, of the \Fix{500}
projects whose tests we re-executed in different OSes we find
that \Fix{4.7\%} of them have tests with portability problems.
\end{tcolorbox}

\subsection{Answering \RQTwo}
...
\begin{table*}[t]
    ...
    & Path separators & Forward vs. backslash differences & 27 & 7 & \multirow{6}{*}{59} \\
    & Line endings & CRLF vs. LF mismatch & 4  & 1 &                      \\
    & Case sensitivity & Filename case handling varies & 1  & 1 &                      \\
    & File locking & Windows locks open files & 4 & 5 &                      \\
    & Encoding & UTF-8 not default everywhere & 7 & 2 &                      \\
    & Filesystem block size & Block size assumptions differ & 1 & 0 &                      \\
    \midrule
    
    \multirow[t]{3}{*}{Process and Signals (PROC)}
    & Shell command execution & Shell behavior differs across OS & 14 & 5 & \multirow{3}{*}{25} \\
    & Missing signals & Signals unavailable on Windows & 2  & 3 &                      \\
    & Address already in use & Port binding conflicts vary & 0  & 1 &                      \\
    \midrule
    
    \multirow[t]{4}{*}{Library Dependencies (LIB)}
    & Platform-specific libraries (e.g. fcntl) & Unix-only libraries missing elsewhere & 1  & 2 & \multirow{4}{*}{24} \\
    & Missing libraries & Dependencies not universally available & 11 & 2 &                      \\
    & Dynamic library loading & DLL vs. SO file issues & 5  & 0 &                      \\
    & Binary wheel mismatch & Architecture-specific wheel incompatibilities & 3 & 0 &                      \\
    \midrule
    
    \multirow[t]{4}{*}{Environment and Display (ENV)}
    & No display in github CI (linux) & Headless CI lacks display server & 2 & 3 & \multirow{4}{*}{14} \\
    & GUI differences & Window manager behavior varies & 4 & 0 &                      \\
    & Missing curses & Curses unavailable on Windows & 0 & 1 &                      \\
    & Terminal capabilities & Terminal features differ across systems & 2 & 2 &                      \\
    \midrule
    
    \multirow[t]{2}{*}{API Availability (API)}
    & Methods: os.uname(), os.geteuid(), os.getuid(), os.getpgid() & OS methods missing on Windows & 2 & 4 & \multirow{2}{*}{8} \\
    & Modules: readline, resource & Optional modules not everywhere & 2 & 0 &                      \\
    \midrule
    
    \multirow[t]{3}{*}{Permissions and Limits (PERM)}
    & Permission & Permission models differ by OS & 1 & 3 & \multirow{3}{*}{7} \\
    & File descriptor limits & Resource limits vary across systems & 1 & 0 &                     \\
    & Symlink privileges & Windows requires admin for symlinks & 1 & 1 &                     \\
    \midrule
    
    \multirow[t]{2}{*}{System Information (SYS)}
    & /proc filesystem & /proc missing on macOS/Windows & 0 & 1 & \multirow{2}{*}{2} \\
    & Timezone database & Timezone data not always installed & 0 & 1 &                      \\
    \midrule
...
While process and signal-related problems (PROC) accounted for only 2 pull requests, they achieved a 100\% acceptance rate from maintainers. When such issues occur, they typically have severe functional impact—crashes, hangs, or incorrect behavior. Developers should test subprocess invocation, signal handling, and process management across platforms early in development, as these issues are immediately recognized as critical by the community.
